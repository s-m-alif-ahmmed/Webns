                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="border p-3 ">
                            <form class="row g-3" method="post" action="{{route('admin-faq.store')}}" enctype="multipart/form-data">
                                @csrf
                                @method('post')
                                ...
                                <div class="col-12  form-group">
                                    ...
                                    <select class="form-control select2 form-select" name="faq_category_id" data-placeholder="Choose FAQ Category" required>
                                        <option value="1" selected>Choose one FAQ category</option>
                                        @foreach($faq_categories as $faq_category)
                                            <option value="{{ $faq_category->id }}" {{$faq_category->faq_id == $faq_category->id ? 'selected' : ''}} >{{ $faq_category->english }} ({{ $faq_category->bangla }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-12">
                                    ...
                                    <input class="form-control" type="text" name="answer" id="answer" placeholder="Enter faq answer" required />
                                </div>

                                <div class="col-12">
                                    <label for="single_image" class="form-label"> FAQ Single Image </label>
                                    <input class="form-control" type="file" name="single_image" id="single_image" placeholder="Choose faq single_image" />
                                </div>

                                <div class="col-12">
                                    <label for="image" class="form-label"> FAQ Multiple Images </label>
                                    <input class="form-control" type="file" name="image[]" id="image" multiple placeholder="Choose faq image" />
                                </div>
